...
extension Passage {
    func showcaseDocuments()->[(Breakdown,String)] {
        ...
        return LibraryStore.records(in:saveURL.deletingLastPathComponent().appendingPathComponent("Library")).filter{ids.contains($0.document.document.id)}.sorted{(ids.firstIndex(of:$0.document.document.id) ?? Int.max) < (ids.firstIndex(of:$1.document.document.id) ?? Int.max)}.map{($0.document,$0.url.path)}
    }
...
final class ThemePicker:NSView {
    ...
    init(selected:String){self.selected = selected;super.init(frame:.zero)
        for (i,name) in names.enumerated(){let b = NSButton(title:"Aa\n" + name,target:self,action:#selector(selectTheme(_:)));b.tag = i;b.bezelStyle = .regularSquare;b.isBordered = false;b.wantsLayer = true;if let layer = b.layer {LiquidGlass.configureLayer(layer,radius:LiquidGlass.smallCornerRadius,shadow:false)};b.font = NSFont(name:"Georgia",size:16);b.setAccessibilityLabel(name + " theme");addSubview(b);buttons.append(b)}
    }
    @available(*, unavailable)
    required init?(coder:NSCoder){fatalError("This view is created programmatically")}
