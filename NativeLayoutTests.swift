...
@main struct NativeLayoutTests {
    ...
    static func main() throws {
        ...
        // Presentation roles are explicit; historical kind=comment stays a margin note.
        app.data=Breakdown.plain("A passage for discussion.",title:"Discussion fixture")
        let legacy=Note(id:"legacy",start:0,end:9,quote:"A passage",kind:"comment",level:"essay",label:"Context",body:"Short context",side:"left")
        app.data.annotations=[legacy]
        app.openWorkspace()
        precondition(!app.isDiscussion(legacy))
        let before=try JSONEncoder().encode(app.data)
        app.notePresentation.set(.comment,documentID:app.data.document.id,noteID:legacy.id)
        precondition(app.isDiscussion(legacy))
        precondition(NotePresentation(defaults:UserDefaults(suiteName:name)!).surface(documentID:app.data.document.id,noteID:legacy.id) == .comment)
        precondition(app.notePresentation.surface(documentID:"other",noteID:legacy.id) == .annotation)
        app.commentsVisible=true;app.adaptLayout();app.renderNotes();app.root.layoutSubtreeIfNeeded()
        precondition(!app.commentsPane.isHidden && app.commentsPane.bounds.width == 300)
        precondition((app.leftNotes.subviews+app.rightNotes.subviews).allSatisfy {$0.identifier?.rawValue != legacy.id})
        let after=try JSONDecoder().decode(Breakdown.self,from:before)
        precondition(app.data.annotations[0].body==after.annotations[0].body && app.data.annotations[0].kind==after.annotations[0].kind)
        app.openCommentPopover(existing:legacy,targetRect:.zero,view:app.editor)
        precondition(app.commentsEditing && app.commentField.superview != nil)
        app.commentField.string="More detailed feedback";app.persistComment();app.closeComment()
        precondition(app.data.annotations.count==1 && app.data.annotations[0].body=="More detailed feedback")
        app.openCommentPopover(existing:app.data.annotations[0],targetRect:.zero,view:app.editor)
        app.moveNoteSurface()
        precondition(!app.isDiscussion(app.data.annotations[0]) && !app.commentsVisible)
        app.editor.setSelectedRange(NSRange(location:10,length:3))
        app.comment()
        app.commentField.string="A new comment";app.persistComment();app.closeComment()
        precondition(app.data.annotations.count==2 && app.data.annotations.filter {app.isDiscussion($0)}.count==1)
        try app.data.validate()
        app.undoEdit()
        precondition(app.data.annotations.count==1,"Undo restores document before creating comment")
        app.redoEdit()
        precondition(app.data.annotations.count==2 && app.data.annotations.filter {app.isDiscussion($0)}.count==1)
        app.root.layoutSubtreeIfNeeded()
        try render(app.root,to:"/tmp/ppb-comments-layout.png")
        // Export bundle carries the comment surface map and re-imports it faithfully.
        let bundleURL=URL(fileURLWithPath:NSTemporaryDirectory()).appendingPathComponent("ppb-bundle-"+UUID().uuidString)
        let share=ShareController(document:app.data,pdf:annotatedPDF(app.data,surfaces:app.commentSurfaces),surfaces:app.commentSurfaces)
        share.format.selectItem(at:1)
        try share.write(to:bundleURL)
        precondition(FileManager.default.fileExists(atPath:bundleURL.appendingPathComponent("annotations.json").path))
        precondition(FileManager.default.fileExists(atPath:bundleURL.appendingPathComponent("presentation.json").path),"Comment surfaces must be exported for portability")
        app.notePresentation.set(.annotation,documentID:app.data.document.id,noteID:app.data.annotations[1].id)
        precondition(!app.isDiscussion(app.data.annotations[1]))
        try app.importWritingBundle(bundleURL)
        precondition(app.isDiscussion(app.data.annotations[1]),"Re-imported bundle must restore comment surfaces")
        let routedPDF=annotatedPDF(app.data,surfaces:app.commentSurfaces)
        let routedText=(0..<routedPDF.pageCount).compactMap {routedPDF.page(at:$0)?.string}.joined()
        precondition(routedText.contains("A new comment"),"Comment body must appear in the comments appendix")
        precondition(routedText.contains("Comments · feedback"),"PDF must include a comments appendix section")
        precondition(FileManager.default.fileExists(atPath:bundleURL.appendingPathComponent("essay.md").path))
        try FileManager.default.removeItem(at:bundleURL)
        print("PASS: legacy note preservation, separate comments pane, edit, conversion, creation and undo/redo")
