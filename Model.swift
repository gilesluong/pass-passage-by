...
struct Breakdown: Codable {
 ...
 static func plain(_ text: String, title: String) -> Self {
  Self(document: Essay(id:UUID().uuidString,title:title,text:text,taskType:"unknown"),annotations:[])
 }
 ...
 func range(level:Int, offset:Int)->NSRange {
  ...
  let pattern = level == 1 ? #"[^\r\n]+(?:\n(?!\s*\n)[^\n]+)*"# : level == 2 ? #"[^.!?\n]+(?:[.!?]+[\"”’']*|$)"# : #"[\p{L}\p{N}]+(?:['’\-][\p{L}\p{N}]+)*"#
  guard let regex = try? NSRegularExpression(pattern:pattern,options:.anchorsMatchLines) else { return NSRange(location:0,length:source.length) }
