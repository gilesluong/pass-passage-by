...
enum LocalOCR {
    ...
    static func recognize(_ pages:[CapturedPage])throws->OCRResult {
        ...
        for page in pages {
            ...
            if #available(macOS 14.0,*) {} else {request.usesCPUOnly=true}
...
enum LocalFeedback {
    ...
    static func anchors(_ suggestions:[SuggestedNote],source:String)throws->[Note] {
        ...
        return try suggestions.prefix(8).enumerated().map {index,s in
            ...
            let kind=["comment","correction","vocabulary","structure"].contains(s.kind ?? "") ? s.kind!:"comment"
            let tag=["blue","orange","red","purple","green","yellow","gray"].contains(s.tag ?? "") ? s.tag!:"blue"
...
final class ScanReview:NSWindowController,NSWindowDelegate,NSTextViewDelegate {
    ...
    init(passage:Passage,pages:[CapturedPage]) {
        ...
        for (i,_) in pages.enumerated(){pagePicker.addItem(withTitle:"Page \(i+1)")};pagePicker.target=self;pagePicker.action=#selector(changePage)
